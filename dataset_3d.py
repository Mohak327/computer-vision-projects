...
def image_to_rays(
    ...
    r_os, r_ds = pixels_to_rays(K=K, c2w=c2w, uvs=uvs, verbose=verbose, device=device)
    rays = torch.cat((r_os, r_ds), dim=1).reshape(h, w, 6)
...
def images_to_rays(
    ...
    rays_per_image = [
        image_to_rays(images[i], c2ws[i], K, verbose=verbose, device=device)
...
class RaysData(Dataset):
    def __init__(
        ...
        self.images = torch.as_tensor(self.images, dtype=torch.float32, device=device)
        self.K = torch.as_tensor(self.K, dtype=torch.float32, device=device)
        self.c2ws = torch.as_tensor(self.c2ws, dtype=torch.float32, device=device)
    ...
    def sample_rays(self, num_rays: int):
        ...
        r_os = self.rays_o[sample_indices]
        r_ds = self.rays_d[sample_indices]
        gt_rgbs = self.gt_rgbs[sample_indices]
